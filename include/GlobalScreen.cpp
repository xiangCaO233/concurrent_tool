...
void GlobalScreen::pressMouse(CGMouseButton &button) {
    ...
    if (button == kCGMouseButtonLeft)triggerMouse(kCGEventLeftMouseDown, current, button);
    else if (button == kCGMouseButtonRight)triggerMouse(kCGEventRightMouseDown, current, button);
    else triggerMouse(kCGEventOtherMouseDown, current, button);
}

void GlobalScreen::releaseMouse(CGMouseButton &button) {
    CGPoint current = CGEventGetLocation(CGEventCreate(nullptr));
    if (button == kCGMouseButtonLeft)triggerMouse(kCGEventLeftMouseUp, current, button);
    else if (button == kCGMouseButtonRight)triggerMouse(kCGEventRightMouseUp, current, button);
    else triggerMouse(kCGEventOtherMouseUp, current, button);
}
...
void GlobalScreen::clickedMouse(CGPoint &point, CGMouseButton &button, long timeContinued) {
    if (button == kCGMouseButtonLeft) {
        ...
        triggerMouse(kCGEventRightMouseDown, point, button);
        if (timeContinued)
            usleep(timeContinued * 1000); // 睡眠
        triggerMouse(kCGEventRightMouseUp, point, button);
    } else {
        triggerMouse(kCGEventOtherMouseDown, point, button);
        if (timeContinued)
            usleep(timeContinued * 1000); // 睡眠
        triggerMouse(kCGEventOtherMouseUp, point, button);
    }
}
...
void
GlobalScreen::clickedAndReturnMouse(CGPoint &srcPoint, CGPoint &desPoint, CGMouseButton &button, long timeContinued) {
    clickedMouse(desPoint, button, timeContinued);
    moveMouse(srcPoint);
}
